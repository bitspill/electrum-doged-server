...
The most up-to date version of this document is available at:
    ...
    https://github.com/doged/electrum-doged-server/blob/master/HOWTO.md
...
document, we assume that user is called 'dogecoindark'). We also assume the
dogecoindark user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run dogecoindarkd and keep a copy of the full blockchain, 
which is roughly 4 GB in April 2014. If you have less than 2 GB of RAM 
make sure you limit dogecoindarkd to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running dogecoindarkd and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `dogecoindarkd` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser dogecoindark --disabled-password
    $ sudo apt-get install git
    $ sudo su - dogecoindark
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/dogecoindark/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download dogecoindarkd

We currently recommend dogecoindarkd 0.10.1.3 or 0.8.7.5 stable.

If you prefer to compile dogecoindarkd, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config automake libtool
    $ sudo su - dogecoindark
    $ cd ~/src && git clone https://github.com/doged/dogedsource.git 
    $ cd dogedsource/src
    $ make -f makefile.unix
    $ strip src/dogecoindarkd src
    $ cp -a src/dogecoindarkd src ~/bin

### Step 3. Configure and start dogecoindarkd

In order to allow Electrum to "talk" to `dogecoindarkd`, we need to set up an RPC
username and password for `dogecoindarkd`. We will then start `dogecoindarkd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.DogeCoinDark
    $ $EDITOR ~/.DogeCoinDark/DogeCoinDark.conf

Write this in `DogeCoinDark.conf`:
...
If you have an existing installation of dogecoindarkd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ ./dogecoindarkd -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `dogecoindarkd`:

    $ ./dogecoindarkd

Allow some time to pass, so `dogecoindarkd` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ ./dogecoindarkd getinfo

Before starting the electrum server your dogecoindarkd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start dogecoindarkd at boot
time, running as the 'dogecoindark' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/doged/electrum-doged-server.git
    $ cd electrum-doged-server
    $ sudo configure
...
and further instructions from the Electrum-DOGED full archival server foundry at:
http://electrum-doged.space/leveldb-dump/
...
Electrum reads a config file (/etc/electrum-doged.conf) when starting up. This
file includes the database setup, dogecoindarkd RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-doged.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-doged-server.
If you're using user dogecoindark to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "dogecoindark hard nofile 65536" >> /etc/security/limits.conf
     echo "dogecoindark soft nofile 65536" >> /etc/security/limits.conf

Two more things for you to consider:

1. To increase security you may want to close dogecoindarkd for incoming connections and connect outbound only

2. Consider restarting dogecoindarkd (together with electrum-doged-server) on a weekly basis to clear out unconfirmed
...
Note: If you want to run the server without installing it on your system, just run 'run_electrum_doged_server" as the
unprivileged user.
...
If your system supports it, you may add electrum-doged-server to the /etc/init.d directory. 
...
$doged's to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-doged and we'll try to congratulate you
on supporting the community by running an Electrum-DOGED node.

If you're operating a public Electrum-DOGED server please subscribe
to the following mailing list:
https://groups.google.com/forum/#!forum/electrum-doged-server
It'll contain announcements about important updates to Electrum-DOGED
